...
var MENU_OPTION_SUPPORT_FAPCTL    =62;
...
function menuSupport(options) {
   ...
   var supportFapctl = options[MENU_OPTION_SUPPORT_FAPCTL]; 
...
//	insDoc(nodeAdvancedSetup, gLnk('R', getMenuTitle(MENU_WAN),'wancfg.cmd'));
		nodeNetwork = insFld(foldersTree, gFld(getMenuTitle(MENU_NETWORK), 'wlswitchinterface0.html'));
	...
	if ( proto != 'Not Applicable' && ipExt != '1' ) {
      if ( tod == '1' ) 
      {
		 nodeAdvancedSetup = insFld(foldersTree, gFld(getMenuTitle(MENU_ADVANCED_SETUP), 'todmngr.tod?action=view'));
//TOANTK Modify Menu Tree: nodeAdvancedSetup
         nodeParentalControl = insFld(nodeAdvancedSetup, gFld(getMenuTitle(MENU_PARENTAL_CNTL),'todmngr.tod?action=view'));
//		 nodeParentalControl = insFld(foldersTree, gFld(getMenuTitle(MENU_PARENTAL_CNTL),'todmngr.tod?action=view'));
...
//      nodeStorage = insFld(foldersTree, gFld(getMenuTitle(MENU_STORAGESERVICE), 'storageservicecfg.cmd?view'));	  
...
//Moved PrinterServer from Advanced Setup	  
	  if ( ipp == '1' )
      nodePrinterServer = insFld(nodeStorage, gFld(getMenuTitle(MENU_IPP), 'ippcfg.html'));
   }
...
//	nodeQos = insFld(foldersTree, gFld(getMenuTitle(MENU_QOS),'qosqmgmt.html'));
...
//Configure VPN class menu   
   ...
   if ( parseInt(numWl) != 0 ) {
      if(numWl != '1')
         wlanMenu = insFld(nodeNetwork, gFld(getMenuTitle(MENU_WIRELESS_SETTINGS), wlItemsCgiCmd[0]));
 
      for(i = 0; i < parseInt(numWl); i++)
      {
         // Configure wireless menu
     /*    if(numWl == '1')
            nodeWireless = insFld(nodeNetwork, gFld(getMenuTitle(MENU_WIRELESS_SETTINGS), wlItemsCgiCmd[0]));
         else
...
//      insFld(nodeMngr, gFld(getMenuTitle(MENU_TL_SNMP), 'snmpconfig.html'));
 ...
 //     insFld(nodeMngr, gFld(getMenuTitle(MENU_TR69C), 'tr69cfg.html'));
   ...
   insDoc(nodeAccCntr, gLnk('R', getMenuTitle(MENU_REMOTE), 'scinflt.cmd?action=remoteview')); //ToanTK

   insFld(nodeMngr, gFld(getMenuTitle(MENU_TL_UPDATE_SOFTWARE), 'upload.html'));
   insFld(nodeMngr, gFld(getMenuTitle(MENU_AUTO_REBOOT), 'autoreboot.html'));				//Tunt
...
//		nodeAdvancedSetup = insFld(foldersTree, gFld(getMenuTitle(MENU_ADVANCED_SETUP), 'wancfg.cmd'));
//	insDoc(nodeAdvancedSetup, gLnk('R', getMenuTitle(MENU_WAN),'wancfg.cmd'));
		nodeNetwork = insFld(foldersTree, gFld(getMenuTitle(MENU_NETWORK), 'modifypassword.html'));		
	insDoc(nodeNetwork, gLnk('R', getMenuTitle(MENU_WAN),'modifypassword.html'));
	}
	if (atm == '1' || ptm == '1') {
		isDsl = 1;
	}
...
//HuyHQ modify: MENU_NETWORK	
//	nodeLAN = insDoc(nodeAdvancedSetup, gFld(getMenuTitle(MENU_LAN),'lancfg2.html'));
	nodeLAN = insDoc(nodeNetwork, gFld(getMenuTitle(MENU_LAN),'lancfg2.html'));
   ...
   // Configure wireless menu
   //TOANTK Modify Menu Tree: nodeNetwork
   if ( parseInt(numWl) != 0 ) {
      if(numWl != '1')
         wlanMenu = insFld(nodeNetwork, gFld(getMenuTitle(MENU_WIRELESS_SETTINGS), wlItemsCgiCmd[0]));
...
//HuyHQ Modify: Remove menu Wireless Bridge
//         insDoc(nodeWireless, gLnk('R', getMenuTitle(MENU_WL_WDS), 'wlwds.cmd?action=view'));
   ...
   insDoc(nodeDiagnostics, gLnk('R', getMenuTitle(MENU_DIAGNOSTICS_INTERFACE),'diag.html'));// add by quyenvd
   insDoc(nodeDiagnostics, gLnk('R', getMenuTitle(MENU_DIAGNOSTICS_TOOL),'diagtool.html'));// add by Tunt
   if (p8021ag == '1') {
...
function createBcmMenu(options) {
   ...
   // device wan menu
   insFld(nodeDeviceInfo, gFld(getMenuTitle(MENU_DEVICE_WAN), 'modifypassword.html'));
   ...
   if (!(proto == 'Bridge' || ipExt == '1') && dhcpen == '1') {
      insFld(nodeDeviceInfo, gFld(getMenuTitle(MENU_REBOOT),'dhcpinfo.html'));
   }
   if ( user == 'admin' )
      menuSupport(options);
      ...
      menuUser();
insFld(foldersTree, gFld(getMenuTitle(MENU_LOGOUT), 'javascript:if (confirm("Do you want to logout?")){document.cookie="Authorization=;expires="+(new Date()).toGMTString();window.top.location.href="index.html";};'));
}
