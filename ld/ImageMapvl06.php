...
<?php 
	//$sql= "SELECT *FROM cdata INNER JOIN  camera1 ON cdata.mac= camera1.mac ";
		$sql = "SELECT mac, time, img FROM camera1 WHERE mac = '06' ORDER BY id DESC LIMIT ";
		...
		file_put_contents("C:\\xampp\\htdocs\\sg\\ImageMap06\\'".$mac.$time."'.jpeg", $binary);
		$folder_path = 'ImageMap06/'; //image's folder path
